...
$(document).ready(function() {
  ...
  var $actorVideoGallery = $('.actorVideoGallery');
  ...
  $('#actorVideo').on('hidden.bs.modal', function (e) {
    $('.videoIframe').each(function(){
      this.contentWindow.postMessage('{"event":"command","func":"stopVideo","args":""}', '*')
    });
  });
  ...
  $('#actorVideo').on('shown.bs.modal', function (e) {
    if ($actorVideoGallery.length) {
      $actorVideoGallery.slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          autoplay: false,
          draggable: false,
